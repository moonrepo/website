...
The [`moon ci`](../commands/ci.mdx) command does all the heavy lifting necessary for effectively running jobs. It achieves this by automatically running the following steps:

- Determines changed files by comparing the current HEAD against a base.
- Determines all [targets](<../(fundamentals)/target.mdx>) that need to run based on changed files.
- Additionally runs affected [targets](<../(fundamentals)/target.mdx>) dependencies _and_ dependents.
- Generates an [action graph](<../(fundamentals)/action-graph.mdx>) (dependency graph).
- Installs the applicable [toolchains](<../(fundamentals)/toolchain.mdx>) for each running task.
- Runs all [actions](<../(fundamentals)/action.mdx>) within the graph using a thread pool.
- Displays stats about all passing, failed, and invalid [actions](<../(fundamentals)/action.mdx>).
...
through [`options.runInCI`](/api/moon/TaskOptionsConfig#runinci).
...
By default [`moon ci`](../commands/ci.mdx) will run _all_ tasks from _all_ projects that are affected by changed files and have [`options.runInCI`](/api/moon/TaskOptionsConfig#runinci) enabled. This is a great catch-all solution, but may not vibe with your workflow or requirements.

If you'd prefer more control, you can pass a list of targets to [`moon ci`](../commands/ci.mdx), instead of moon
attempting to detect them. When providing targets, [`moon ci`](../commands/ci.mdx) will still only run them if affected by
changed files, but will still filter with [`options.runInCI`](/api/moon/TaskOptionsConfig#runinci).
...
[`vcs.defaultBranch`](/api/moon/VcsConfig#defaultbranch) for the base revision, and `HEAD` for the
head revision.
...
When these options are passed, moon will only run affected [targets](<../(fundamentals)/target.mdx>) based on
