...
- Generates an [action graph](../action-graph) (dependency graph).
...
By default [`moon ci`](../commands/ci) will run _all_ tasks from _all_ projects that are affected by changed files and have [`options.runInCI`](/api/moon/TaskOptionsConfig#runinci) enabled. This is a great catch-all solution, but may not vibe with your workflow or requirements.
