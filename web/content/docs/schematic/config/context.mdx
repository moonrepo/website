...
Context is an important mechanism that allows for different [default values](./struct/default.mdx),
[merge strategies](./struct/merge.mdx), and [validation rules](./struct/validate.mdx) to be used, for
...
<Callout>
  Refer to the [default values](./struct/default.mdx), [merge
  strategies](./struct/merge.mdx), and [validation rules](./struct/validate.mdx)
  sections for more information on how to use context.
</Callout>

## Metadata

Alongside the configuration itself, the derive records a little metadata about each
[setting](./settings.mdx), reachable with
...
`nested` map when the setting holds another [nested config](./nested.mdx).

<Callout>
  A key derived from an [`env_prefix`](./struct/env.mdx#container-prefixes) is
  reported with the container's own prefix. A parent's `#[setting(nested,
  env_prefix)]` override is applied at runtime, so it isn't reflected here.
</Callout>
