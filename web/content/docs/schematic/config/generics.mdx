...
[`Config`](./struct/index.mdx), [`ConfigEnum`](./enum/index.mdx), and
[`Schematic`](../schema/index.mdx) can all be derived for types with type parameters. The parameters
are carried into the [partial](./partial.mdx) and into every generated implementation.
...
[defining names](../schema/types.mdx#defining-names) for the manual equivalent.

## Enums

A generic [`ConfigEnum`](./enum/index.mdx) only makes sense with a
[fallback variant](./enum/fallback.mdx), since unit variants carry no data. The parameter needs
