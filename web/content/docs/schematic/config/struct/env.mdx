...
<Callout>
  Requires the `env` Cargo feature, which is enabled by default. Not supported
  for enums.
</Callout>
...
<Callout>
  ...
  [schema](../../schema/index.mdx). A derived key appears in a generated [config
  template](../../schema/generator/template.mdx) and in
...
<Callout>
  We provide a handful of built-in parsing functions in the [`env`
  module](https://docs.rs/schematic/latest/schematic/env/index.html).
</Callout>
