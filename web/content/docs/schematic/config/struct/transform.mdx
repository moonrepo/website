...
<Callout>
Transformations happen during the finalize phase, _after_ [environment variables](./env.mdx) are
inherited, and _before_ it is [validated](./validate.mdx).
</Callout>

## Transform handler function

When defining a custom `transform` function, the defined value and [context](../context.mdx) are
...
Transformers can also be used on [nested configs](../nested.mdx), but when defining the transformer
function, the value being transformed is the _[partial nested config](../partial.mdx)_, not the final
one. For example:
