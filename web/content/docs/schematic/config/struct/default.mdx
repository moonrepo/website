...
- The first is applied through the [partial configuration](../partial.mdx), is defined with the
  `#[setting]` attribute, and is the first layer to be merged.
- The second is on the [final configuration](../index.mdx) itself, and uses the `Default` trait to
...
<Callout>
  A handler function that must keep a non-conventional name can be reached
  through a `snake_case` wrapper, or called directly with `default =
  my_wrapper()` when it needs no context.
</Callout>
...
function to call. This function receives the [context](../context.mdx) as the first argument, and can
...
If you're not using [context](../context.mdx), you can use `()` as the context type, or rely on
generic inferrence.
