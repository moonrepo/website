...
for each [partial configuration](../partial.mdx) to be merged. However, with that said, _prefer serde
...
<Callout>
  We provide a handful of built-in validation functions in the [`validate`
  module](https://docs.rs/schematic/latest/schematic/validate/index.html).
  Furthermore, some functions are factories which can be called to produce a
  validator.
</Callout>
...
current/parent partial as the second, the [context](../context.mdx) as the third, and whether this is
...
If you're not using [context](../context.mdx), or want to create a validator for any kind of partial,
