...
derived [`Config` struct](./struct/index.mdx) or [`Config` enum](./struct/index.mdx), with all
...
For example, the `ExampleConfig` from the [first chapter](../config/index.mdx) would generate the
...
extending, and merging process, and ultimately become the [final configuration](./index.mdx).
...
- Defining [default values](./struct/default.mdx) for settings.
- Inheriting [environment variable](./struct/env.mdx) values.
- Merging partials with [strategy functions](./struct/merge.mdx).
- Validating current values with [validate functions](./struct/validate.mdx).
- Declaring [extendable sources](./struct/extend.mdx).
