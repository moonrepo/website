...
[`Config` structs](./struct/index.mdx) can easily be nested within other [`Config`][config]s using
...
its [partial](./partial.mdx) implementation.

<Callout>
  Nested values can also be wrapped in collections, like `Vec` and `HashMap`.
