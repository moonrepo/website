...
The [`EnumType`][enum] can be used to represent a list of [literal values](./literal.mdx).
...
<Callout>
  `variants` is the source of truth, and `values` is a derived subset of it. A
  variant whose schema holds no literal value contributes no entry to `values`,
  so the two can differ in length.
</Callout>
