...
<Callout>Requires the `json_schema` Cargo feature.</Callout>
...
<Callout>
  ...
  [moonrepo/moon](https://github.com/moonrepo/moon/blob/master/crates/config/src/main.rs).
</Callout>
...
<Callout>
  ...
  type in the `schemars` crate — `meta_schema`, `transforms`, and
  `inline_subschemas`. Refer to their documentation for more information.
</Callout>

<Callout>
  `definitions_path` is the prefix each `$ref` is built from, such as
  `#/definitions/`. Schemars models its own field of that name as a JSON pointer
