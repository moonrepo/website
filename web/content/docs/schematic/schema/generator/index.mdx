...
<Callout>
  We'll recursively add referenced and nested schemas for types that are added.
  No need to explicitly add all required types!
</Callout>

Schemas are keyed by [name](../types.mdx#defining-names), so adding two different types that report
...
- [API documentation](./api-docs.mdx)
- [Config templates](./template.mdx)
- [JSON schemas](./json-schema.mdx)
- [TypeScript types](./typescript.mdx)
