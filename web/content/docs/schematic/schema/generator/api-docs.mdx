...
<Callout>Requires the `renderer_api_docs` Cargo feature.</Callout>
...
Like a [JSON schema](./json-schema.mdx), each render produces a single document, and the _last type
...
<Callout>
  Tags in the table are rendered as labels, not through the `render_tags`
  function, as its output is a block that cannot sit inside a table cell.
</Callout>
