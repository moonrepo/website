...
<Callout>Requires the `typescript` Cargo feature.</Callout>
...
[Enum types](../enum.mdx) can be rendered in a format of your choice using the `enum_format` option
and the
...
[Struct types](../struct.mdx) can be rendered as either TypeScript interfaces or type aliases using
