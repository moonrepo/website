...
<Callout>
  Requires the `template` and desired
  [format](../../config/index.mdx#supported-source-formats) Cargo feature.
</Callout>
...
<Callout type="warning">The `format` option is required!</Callout>
...
<Callout type="warning">
  If the custom schema type does not match the field type, it will trigger a
  panic.
</Callout>
