...
<Callout>
  Automatically implemented for `usize`-`u128`, `isize`-`i128`, and their
  `NonZero` counterparts. Unsigned `NonZero` types carry a `min` of 1; signed
  ones cannot express "not zero" as a bound, so they stay unconstrained.
</Callout>

## Default value

To customize the default value for use within [generators](./generator/index.mdx), pass the desired
...
<Callout>This is primarily used by JSON Schema.</Callout>
